...
class IGFeedPostActionsTableViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        sendButton.addTarget(self, action: #selector(didTapSedButton), for: .touchUpInside)
    }
    ...
    @objc private func didTapLikeButton() {
        isLike = !isLike
        let config = UIImage.SymbolConfiguration(pointSize: 25, weight: .thin)
        let image = UIImage(systemName: isLike ? "heart.fill" : "heart", withConfiguration: config)
    ...
    @objc private func didTapSedButton() {
